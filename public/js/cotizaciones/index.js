var cotizaciones_table = $('#cotizaciones-table').DataTable({
    "responsive": true,
    "processing": true,
    "info": true,
    "showNEntries": true,
     ...
     
    {
        ...
        "render": function(data, type, full, meta) {
            ...
            if(full.estado_id == 1){
                ...
                "<i class='fa fa-btn fa-eye' title='Ver Cotización'></i>" + 
                "</a>" + "</div>" + 
                "<div class='float-center col-lg-3'>" + 
                "<a href='/cotizaciones/pdf/" + full.id + "' class='pdfcotizacion'" + "' target='_blank'" + ">" +
                "<i class='fas fa-print' title='Imprimir Cotización'></i>" +
                "</a>" + "</div>" +
                "<div class='float-center col-lg-3'>" + 
                "<a href='/cotizaciones/edit/" + full.id + "' class='edit-cotizacion' >" + 
                "<i class='fa fa-btn fa-edit' title='Editar Cotización'></i>" +
                "</a>" + "</div>" + 
                "<div class='float-right col-lg-3'>" + 
                "<a href='#' class='remove-cotizacion' data-method='delete' data-id='"+full.id+"' data-target='#modalConfirmarAccion' data-toggle='modal'>" + 
                "<i class='fas fa-trash-alt' title='Eliminar Cotización'></i>" + 
                "</a>" + "</div>";
                ...
                if(rol_user == 'Super-Administrador' || rol_user == 'Administrador'){
                    ...
                    "<a href='"+urlActual+"/"+full.id+"/activar' class='activar-cotizacion'"+ "data-method='post' data-id='"+full.id+"' >" + 
                    "<i class='fa fa-thumbs-up' title='Activar Cotización'></i>" + 
                    "</a>" + "</div>";
                }else{
...
function confirmarAccion(button) {
	...
	$.ajax({
		...
		url: APP_URL+"/cotizaciones/" + id + "/delete",
		data: formData,
		dataType: "json",
		success: function(data) {
			...
			alertify.success('La Cotización se Eliminó Correctamente!!');
		},
