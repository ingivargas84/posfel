...
class Fcamel{
    ...
    public function generarFEL($idFacturaMaestro, $idCliente){
        ...
        if ($facturaMaestro->exenta == "No"){
            $frases->setTipoFrase(1);
            $frases->setCodigoEscenario(1);
        }else{
            $frases->setTipoFrase(4);
            $frases->setCodigoEscenario(11);
        }
        ...
        foreach ($facturaDetalle as $item) {
            ...
            if($facturaMaestro->exenta == "No"){
                ...
                $impuestos_detalle = new ImpuestosDetalle();
                $impuestos_detalle->setNombreCorto("IVA");
                $impuestos_detalle->setCodigoUnidadGravable(2);
                $impuestos_detalle->setMontoImpuesto(0);
                $impuestos_detalle->setMontoGravable($granTotal-$facturaMaestro->descuento_porcentaje-$facturaMaestro->descuento_valores);
     ...
     if($facturaMaestro->exenta == "No"){
        ...
        $total_impuestos->setTotalMontoImpuesto(0);
     }  

     $documento_fel->setImpuestosResumen($total_impuestos);

     $totales = new Totales();
     $totales->setGranTotal($granTotal-$facturaMaestro->descuento_porcentaje-$facturaMaestro->descuento_valores);
