            ...
            <div class="box-body">
                <div class="row">
                    ...
                    <div class="col-md-3 col-sm-3">
                        <label for="serie_cotizacion_id">Serie de Cotización:</label>
                        <select name="serie_cotizacion_id" class="form-control" id="serie_cotizacion_id" tabindex="1">
                            <option value="default">Seleccione una Cotización</option>
                            @foreach ($series_cotizacion as $sc)
                            <option value="{{$sc->id}}">{{$sc->serie}}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="col-md-3 col-sm-3">
                        <label for="coti">Núm de Cotización:</label>
                        <input type="number" class="form-control" name="coti" placeholder="No Cotización" id="coti" tabindex="2">
                    </div>
                    <div class="col-md-2 col-sm-2">
                        <label for="coti_id">Id Cotización:</label>
                        <input type="number" class="form-control" name="coti_id" placeholder="Id Cotización" id="coti_id" tabindex="2">
                    </div>
                </div>
                <br>
                <div class="row">
                ...
                <div class="row">
                    ...
                    <div class="col-md-2 col-sm-2">
                        ...
                        <input type="hidden" class="form-control" name="cliente_id2" id="cliente_id2" tabindex="6">
                    </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-4 col-sm-4">
                        ...
                        <input type="text" class="form-control" name="porcentaje" placeholder="Porcentaje Descuento" id="porcentaje" tabindex="11">
                    </div>
                    <div class="col-md-4 col-sm-4">
...
<script>
	...
	var serie_id = $("#serie_cotizacion_id").val();
    var num_coti = $("#coti").val();
	var url = "/facturas/getCotizacion/" + serie_id + "/" + num_coti ;
	if (num_coti != "") {
		$.getJSON( url , function ( result ) {
            $("input[name='coti_id'] ").val(result[0].id);
			$("input[name='nit_cliente'] ").val(result[0].nit);
            $("input[name='cliente_id2'] ").val(result[0].cliente_id);
            $("input[name='nombre_cliente'] ").val(result[0].nombre_comercial);
            $("input[name='porcentaje'] ").val(result[0].porcentaje);
            $("input[name='descuento_valores'] ").val(result[0].descuento_valores);
            ...
            for(i=0; i<$filas; i++)
            {
                detalle_table.row.add({
                    ...
                    'subtotal': result[i].precio_unitario * result[i].cantidad,
                }).draw();
            }
    ...
    $(document).on('focusout', '#precio_unitario', function() {
        $subtotal = parseInt($('#cantidad').val())*parseFloat($('#precio_unitario').val())
    ...
    $(document).on('click', '#ButtonFactura', function(e) {
        ...
        $desc =  ((parseFloat($('#total').val()) * parseFloat($('#porcentaje').val())) /100) + parseFloat($('#descuento_valores').val());
        if ($('#FacturaForm').valid()) {
            ...
            $.ajax({
                ...
                error: function(error) {
                    ...
                    alertify.set('notifier', 'position', 'top-center');
                    alertify.error('Hubo un error, los descuentos son mayores al total de la factura')
            }
        }
    });
